...
void FsCoreImplWindows::addReadOp( FsFileImpl* pImpl, BcSize Position, void* pData, BcSize Bytes, FsFileOpCallback DoneCallback )
{
	BcAssert( Bytes > 0 );
...
void FsCoreImplWindows::addWriteOp( FsFileImpl* pImpl, BcSize Position, void* pData, BcSize Bytes, FsFileOpCallback DoneCallback )
{
...
void FsCoreImplWindows::updateFileMonitoring()
{
	...
	else
	{
		...
		else
		{
			...
			{
				PSY_LOG( "FsCoreImplWindows: File deleted: %s\n", FileName.c_str() );
