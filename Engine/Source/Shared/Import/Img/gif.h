...
// pretty
// much a straight-ahead implementation of the GIF format with optional
// Floyd-Steinberg
// dithering. (It does at least use delta encoding - only the changed portions
// of each
// frame are saved.)
//
...
// reverse order of mallocs
// and any temp memory allocated by a function will be freed before it exits.
// MALLOC and FREE are used only by GifBegin and GifEnd respectively (to
// allocate a buffer the size of the image, which
...
// averaging the blocks at the bottom.
...
void GifDitherImage(const uint8_t* lastFrame, const uint8_t* nextFrame,
  ...
  // to be propagated
  int32_t* quantPixels =
      (int32_t*)GIF_TEMP_MALLOC(sizeof(int32_t) * numPixels * 4);
...
// GIF.
// Many if not most viewers will still display a GIF properly if the EOF code is
// missing,
