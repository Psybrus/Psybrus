...
// ============================================================================
// Ctor
exSsFileWAV::exSsFileWAV( exFileTicket FileTicket, exSsFileDelegate FileDelegate ):
	Header_( EX_NULL ),
	Format_( EX_NULL ),
	Data_( EX_NULL ),
	IsLoaded_( exFalse ),
	LoadCallbackDone_( exFalse )
{
	FileTicket_ = FileTicket;
	FileTicket_.Load( exFileDelegate::Bind< exSsFileWAV, &exSsFileWAV::OnLoadCallback >( this ), EX_NULL );
	FileDelegate_ = FileDelegate;
}

// ============================================================================
// Dtor
//virtual
exSsFileWAV::~exSsFileWAV()
{
	// Block until the load callback has been hit.
	while( LoadCallbackDone_ == exFalse )
	{
		exYield();
	}
...
// ============================================================================
// IsLoaded
exBool exSsFileWAV::IsLoaded() const
{
	return IsLoaded_;
}
...
// ============================================================================
// DataFormat
exSsDataFormat exSsFileWAV::DataFormat() const
{
	exAssert( IsLoaded() == exTrue, "exSsFileWAV: Not loaded yet." );
	return EXSSDF_PCM16;	// TODO: Support other formats.	
}

// ============================================================================
// NumChannels
exU32 exSsFileWAV::NumChannels() const
{
	exAssert( IsLoaded() == exTrue, "exSsFileWAV: Not loaded yet." );
...
// ============================================================================
// SampleRate
exU32 exSsFileWAV::SampleRate() const
{
	exAssert( IsLoaded() == exTrue, "exSsFileWAV: Not loaded yet." );
...
// ============================================================================
// DecodeNextBuffer
exBool exSsFileWAV::DecodeNextBuffer( void*& Data, exU32& DataSize )
{
	exAssert( IsLoaded() == exTrue, "exSsFileWAV: Not loaded yet." );
	Data = ( Data_ + 1 );
	DataSize = Data_->ChunkSize_ - sizeof( Data_ );
	return exTrue;
}

// ============================================================================
// LoadCallback
void exSsFileWAV::OnLoadCallback( exFileTicket FileTicket, void* UserData )
{
	exAssert( FileTicket == FileTicket_, "exSsFileWAV: File ticket does not match." );

	// Setup pointers.
	exU8* FileData = (exU8*)FileTicket_.Data();
	exU32 FileDataSize = FileTicket_.Size();
	...
	// Success.
	IsLoaded_ = exTrue;
	LoadCallbackDone_ = exTrue;

	// Call delegate.
	if( FileDelegate_.Valid() )
	{
		FileDelegate_( this );
	}
	return;
...
// ============================================================================
// FindChunk
exSsFileWAV::Chunk* exSsFileWAV::FindChunk( exConstChar* ChunkName )
{
	exU8* FileData = (exU8*)( Header_ + 1 );
	exU32 FileDataSize = Header_->ChunkSize_;

	Chunk* CurrChunk = reinterpret_cast< Chunk* >( FileData );

	while( ChunkNameTest( CurrChunk, ChunkName ) == exFalse && (exU8*)CurrChunk < ( FileData + FileDataSize ) )
	{
		CurrChunk = reinterpret_cast< Chunk* >( (exU8*)CurrChunk + ( CurrChunk->ChunkSize_ + sizeof( Chunk ) ) );
	}

	if( ChunkNameTest( CurrChunk, ChunkName ) == exTrue )
	{
		return CurrChunk;
	}
...
// ============================================================================
// FindHeaderChunk
exSsFileWAV::HeaderChunk* exSsFileWAV::FindHeaderChunk( exConstChar* ChunkName, exConstChar* HeaderName )
{
	exU8* FileData = (exU8*)FileTicket_.Data();
	exU32 FileDataSize = FileTicket_.Size();

	HeaderChunk* CurrChunk = reinterpret_cast< HeaderChunk* >( FileData );

	if( ChunkNameTest( CurrChunk, ChunkName ) == exTrue && ChunkHeaderNameTest( CurrChunk, HeaderName ) == exTrue )
	{
		return CurrChunk;
	}
...
// ChunkNameTest
//static
exBool exSsFileWAV::ChunkNameTest( Chunk* ChunkData, exConstChar* ChunkName )
{	
	exU32 CharCount = exString::StrLen( ChunkName );
	exAssert( CharCount <= 4, "exSsFileWAV: Chunk name is too long." );
	
	for( exU32 iChar = 0; iChar < CharCount; ++iChar )
	{
...
// ChunkHeaderNameTest
//static
exBool exSsFileWAV::ChunkHeaderNameTest( HeaderChunk* ChunkData, exConstChar* ChunkName )
{	
	exU32 CharCount = exString::StrLen( ChunkName );
	exAssert( CharCount <= 4, "exSsFileWAV: Chunk header name is too long." );
