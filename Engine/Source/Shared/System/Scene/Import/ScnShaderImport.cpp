...
void ScnShaderImport::regenerateShaderDataHeader()
{
	...
	OutString += "#ifndef __PSYBRUSUNIFORMS_PSF__\n";
	OutString += "#define __PSYBRUSUNIFORMS_PSF__\n\n";
	...
	std::array< const char*, 3 > UniformBufferDecls =
	{
		nullptr,
	...
	for( size_t Idx = 0; Idx < OutputBackends.size(); ++Idx )
	{
		...
		auto UniformBufferDecl = UniformBufferDecls[ Idx ];
		...
		for( const auto* Class : ShaderDataClasses )
		{
			CsResourceImporter::addDependency( Class );
	...
	OutString += "#endif // __PSYBRUSUNIFORMS_PSF__\n";
	...
	const char* FileName = "./Content/Engine/PsybrusUniforms.psh";
	if( BcFileSystemExists( FileName ) )
	{
		BcFile ExistingHeader;
		if( ExistingHeader.open( FileName, bcFM_READ ) )
		{
	...
	{
		BcFile OutputHeader;
		if( OutputHeader.open( FileName, bcFM_WRITE_TEXT ) )
		{
