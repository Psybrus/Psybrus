...
{
	BcAssertMsg( ( Texture->getDesc().BindFlags_ & RsBindFlags::RENDER_TARGET ) != RsBindFlags::NONE,
		"Can't bind texture as render target, was it created with RsBindFlags::RENDER_TARGET?" );
...
{
	BcAssertMsg( ( Texture->getDesc().BindFlags_ & RsBindFlags::DEPTH_STENCIL ) != RsBindFlags::NONE,
		"Can't bind texture as depth stencil target, was it created with RsBindFlags::DEPTH_STENCIL?" );
