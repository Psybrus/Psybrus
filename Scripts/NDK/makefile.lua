...
newaction {
	...
	onproject = function(prj)
		...
		for cfg in premake.eachconfig(prj) do
			...
			--	error('The only supported platform for NDK builds is "android", not ' .. cfg.platform)
			--end		
...
function ndk.generateAppMakefile(prj, cfg)
	...
	_p('# Android NDK application makefile autogenerated by GENie')
	_p('PROJECT_PATH := .')
	_p('')

	-- TEST.
	cfg.framework = "android-21"
	cfg.stl = {
		"c++_static"
	}
	...
	if cfg.optimize == 'Off' then
		...
		_p('NDK_TOOLCHAIN_VERSION := clang')
	_p('')			
end
...
function ndk.writeDependencies(location, depends, cfg)
	for _,d in ipairs(depends) do
		if ndk.isValidProject(d) then	
			local p = premake.project.getfilename(d, ndk.getMakefileName(d, cfg, ndk.MAKEFILE))
			p = path.getrelative(location, p)
			_p('include $(DEPENDENCY_PATH)/'..premake.esc(p))
		end
	end
end
...
function ndk.writeRelativePaths(tag, location, paths, local_path)
	...
	for i,p in ipairs(paths) do
		paths[i] = premake.esc(path.getrelative(location, p))
	end
...
function ndk.getDependentModules(prj, cfg, kind)
	local names = {}
	for _,d in ipairs(premake.getdependencies(prj)) do
		if d.kind == kind then
...
function ndk.generateMakefile(prj, cfg)
	...
	for _,d in ipairs(cfg.defines) do
		table.insert( localCFlags, '-D' .. d )
	end

	for _,bo in ipairs(cfg.buildoptions) do
		table.insert( localCFlags, bo )
	end

	table.insert( localCFlags, '-std=c++11' )

	ndk.writeStrings('LOCAL_CFLAGS', '', localCFlags)
	...
	_p('# Include paths')
	local includes = {}
	for _,f in ipairs(cfg.includedirs) do
		f = path.join(prj.location, f)
		table.insert(includes, f)
	end

	ndk.writeRelativePaths('LOCAL_C_INCLUDES', local_path .. '/..', includes, false)
	_p('')
	...
	if cfg.kind == "WindowedApp" then
		ndk.writeDependencies(local_path, premake.getdependencies(prj), cfg)
	end
	_p('')
